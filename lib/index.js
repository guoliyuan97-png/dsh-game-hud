...
export function apply(ctx, config = {}) {
  ...
  const maxBalance = Number.isFinite(Number(config.maxBalance)) && Number(config.maxBalance) > 0
    ? Number(config.maxBalance)
    : 20
  const userPriceTable = config.priceTable && typeof config.priceTable === 'object' && !Array.isArray(config.priceTable)
    ? config.priceTable
    : {}
  const effectivePriceTable = { ...PRICE_TABLE }
  for (const key of Object.keys(userPriceTable)) {
    const entry = userPriceTable[key]
    if (entry && typeof entry === 'object' && entry.peak && entry.valley) {
      effectivePriceTable[key.toLowerCase()] = {
        peak: {
