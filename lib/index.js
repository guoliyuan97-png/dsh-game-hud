...
// 官方高峰时段：北京时间周一至周五 09:00-12:00、14:00-18:00；其余（含周末全天）为空闲时段。
const PEAK_WINDOWS = [[9 * 60, 12 * 60], [14 * 60, 18 * 60]]
// 官方价格（元 / 百万 tokens），来源 https://api-docs.deepseek.com/zh-cn/quick_start/pricing
// 2026-09-10 12:00（北京时间）起 flash 系列降价：空闲 缓存命中/未命中/输出 = 0.02 / 1 / 4，高峰为空闲的 2 倍。
const FLASH_PRICES = {
  peak: { input: 2.0, output: 8.0, cacheHit: 0.04 },
  valley: { input: 1.0, output: 4.0, cacheHit: 0.02 },
}
const PRICE_TABLE = {
  // deepseek-v4-flash / deepseek-v4-flash-vision-exp 与新的官方模型名 deepseek-flash 同价。
  'deepseek-v4-flash': FLASH_PRICES,
  'deepseek-flash': FLASH_PRICES,
  'deepseek-v4-pro': {
...
export function apply(ctx, config = {}) {
  ...
  function pricingState(model) {
    ...
    const mins = now.mins
    const peakDay = isPeakDay(now.day)
    let isPeak = false
    ...
    if (nextSwitchMinutes == null) {
      // 高峰日：找今天剩下的第一个高峰起点
      if (peakDay) {
        for (let i = 0; i < PEAK_WINDOWS.length; i++) {
          const start = PEAK_WINDOWS[i][0]
          if (start > mins) {
            nextSwitchMinutes = start - mins
            nextIsPeak = true
            nextTime = fmtHM(start)
            break
          }
        }
      }
      // 今天已无高峰起点（周末 / 周五 18:00 之后）→ 下一个工作日 09:00
      if (nextSwitchMinutes == null) {
        let addDays = 0
        let d = now.day
        do {
        ...
        const start = PEAK_WINDOWS[0][0]
        nextSwitchMinutes = addDays * 24 * 60 - mins + start
        nextIsPeak = true
        nextTime = weekdayName(d) + ' ' + fmtHM(start)
      }
    }
    ...
    if (key) {
      ...
      const ks = Object.keys(priceTable)
      for (let i = 0; i < ks.length; i++) {
        if (key.indexOf(ks[i]) !== -1) { table = priceTable[ks[i]]; break }
      }
    }
    ...
    return {
      ...
      nextTime,
      nextIsPeak,
      peakWindows: ['周一至周五 09:00-12:00', '周一至周五 14:00-18:00'],
      tz: 'UTC+8',
    }
  }
