...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      // Settings → Plugins → dsh-game-hud card: edit maxBalance (and optionally
      // priceTable) in the DSH settings UI. Changes persist to the profile
      // settings file and apply live to the HUD (no restart).
      const settingsScopeSvc = ctx.get('settingsScope')
      if (settingsScopeSvc !== undefined) {
        const scope = settingsScopeSvc.bind({ namespace: 'game-hud' })

        function HudSettingsCard() {
          const subscribe = React.useMemo(() => scope.subscribe.bind(scope), [])
          const getSnapshot = React.useMemo(() => scope.getSnapshot.bind(scope), [])
          const snap = React.useSyncExternalStore(subscribe, getSnapshot)
          const current = snap && snap.value && typeof snap.value === 'object' ? snap.value : {}
          ...
          React.useEffect(function () {
            const v = current.maxBalance
            setDraft(v != null ? String(v) : '20')
            const t = current.lowThreshold
            setAlertDraft(t != null ? String(t) : '10')
            setAlertOn(current.lowAlert !== false)
            setSynced(true)
          }, [current.maxBalance, current.lowThreshold, current.lowAlert])

          const writable = !snap || snap.writable !== false
          const dirty = synced && (
            draft !== String(current.maxBalance != null ? current.maxBalance : 20) ||
            alertDraft !== String(current.lowThreshold != null ? current.lowThreshold : 10) ||
            alertOn !== (current.lowAlert !== false)
          )
          ...
          const onSave = async function () {
            ...
            const t = Number(alertDraft)
            if (!Number.isFinite(t) || t < 1 || t > 99) {
              setError('告警阈值需在 1–99 之间')
              return
            }
            setSaving(true)
            setError('')
            setSavedMsg('')
            try {
              await scope.set('maxBalance', n)
              await scope.set('lowAlert', alertOn)
              await scope.set('lowThreshold', t)
              setSavedMsg('已保存（HUD 实时生效）')
            } catch (e) {
              setError(e && e.message ? String(e.message) : '保存失败')
            } finally {
              setSaving(false)
            }
          }
          ...
          const onDiscard = function () {
            if (saving) return
            setDraft(current.maxBalance != null ? String(current.maxBalance) : '20')
            setAlertDraft(current.lowThreshold != null ? String(current.lowThreshold) : '10')
            setAlertOn(current.lowAlert !== false)
            setSavedMsg('')
            setError('')
          }

          const cardCss = '.dsh-hud-card{list-style:none;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-3);border-radius:12px;transition:border-color .16s,background .16s}' +
            ...
            '.dsh-hud-card-header{appearance:none;width:100%;font:inherit;color:inherit;text-align:left;cursor:pointer;background:0 0;border:0;border-radius:12px;align-items:center;gap:12px;padding:14px 16px;display:flex}' +
            '.dsh-hud-card-header:focus-visible{outline:2px solid var(--dsw-alias-brand-primary);outline-offset:-2px}' +
            '.dsh-hud-card-headText{flex-direction:column;flex:1;gap:4px;min-width:0;display:flex}' +
            '.dsh-hud-card-name{color:var(--dsw-alias-label-primary);font-size:15px;font-weight:600;line-height:1.4}' +
            '.dsh-hud-card-description{color:var(--dsw-alias-label-tertiary);font-size:13px;line-height:1.5}' +
            '.dsh-hud-card-chevron{color:var(--dsw-alias-label-tertiary);flex:none;width:14px;height:14px;position:relative;transition:transform .16s}' +
            '.dsh-hud-card-chevron::before{content:"";position:absolute;top:3px;left:2px;width:7px;height:7px;border-right:1.6px solid currentColor;border-bottom:1.6px solid currentColor;transform:rotate(45deg)}' +
            '.dsh-hud-card-chevron.open{transform:rotate(180deg)}' +
            '.dsh-hud-card-pending{white-space:nowrap;background:var(--dsw-alias-bg-module-platform);color:var(--dsw-alias-label-secondary);border-radius:999px;flex:none;padding:1px 8px;font-size:11px;font-weight:500;line-height:17px}' +
            '.dsh-hud-card-body{border-top:1px solid var(--dsw-alias-border-l2);margin:0 16px;padding:12px 0 8px}' +
            '.dsh-hud-card-row{display:flex;align-items:center;gap:8px;flex-wrap:wrap}' +
            '.dsh-hud-card-row+.dsh-hud-card-row{margin-top:10px}' +
            '.dsh-hud-card-label{min-width:0;color:var(--dsw-alias-label-primary);font-size:13px;font-weight:500;line-height:1.5;white-space:nowrap}' +
            '.dsh-hud-card-input{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-3);height:34px;font:inherit;color:var(--dsw-alias-label-primary);border-radius:8px;padding:0 12px;font-size:13px;line-height:1.5;width:90px}' +
            '.dsh-hud-card-input:focus-visible{border-color:var(--dsw-alias-brand-primary);outline:none}' +
            '.dsh-hud-card-input:disabled{color:var(--dsw-alias-label-tertiary);cursor:default}' +
            '.dsh-hud-card-switch{position:relative;width:36px;height:20px;border-radius:10px;background:rgba(255,45,85,.25);border:1px solid rgba(255,45,85,.5);cursor:pointer;transition:background .2s;flex:none}' +
            '.dsh-hud-card-switch.on{background:rgba(255,45,85,.75)}' +
            '.dsh-hud-card-switch::after{content:"";position:absolute;top:2px;left:2px;width:14px;height:14px;border-radius:50%;background:#fff;transition:left .2s}' +
            '.dsh-hud-card-switch.on::after{left:18px}' +
            '.dsh-hud-card-hint{color:var(--dsw-alias-label-tertiary);margin:10px 0 0;font-size:12px;line-height:1.5}' +
            '.dsh-hud-card-msg{margin:8px 0 0;font-size:12px;line-height:1.5}' +
            '.dsh-hud-card-msg.ok{color:var(--dsw-alias-state-success-primary)}' +
            '.dsh-hud-card-msg.err{color:var(--dsw-alias-state-error-primary)}' +
            '.dsh-hud-card-footer{border-top:1px solid var(--dsw-alias-border-l2);justify-content:flex-end;align-items:center;gap:8px;padding:12px 0 4px;display:flex}' +
            '.dsh-hud-card-discard,.dsh-hud-card-save{appearance:none;font:inherit;cursor:pointer;border:1px solid transparent;border-radius:8px;padding:5px 14px;font-size:13px;line-height:1.5}' +
            '.dsh-hud-card-discard{border-color:var(--dsw-alias-border-l2);color:var(--dsw-alias-label-secondary);background:0 0}' +
            '.dsh-hud-card-discard:hover:not(:disabled){color:var(--dsw-alias-label-primary);border-color:var(--dsw-alias-label-dimmed)}' +
            '.dsh-hud-card-save{background:var(--dsw-alias-label-primary);color:var(--dsw-alias-bg-layer-3)}' +
            '.dsh-hud-card-discard:disabled,.dsh-hud-card-save:disabled{opacity:.4;cursor:default}' +
            '.dsh-hud-card-discard:focus-visible,.dsh-hud-card-save:focus-visible{outline:2px solid var(--dsw-alias-brand-primary);outline-offset:1px}'

          const bodyId = 'dsh-hud-settings-body'

          return React.createElement('li', { className: 'dsh-hud-card' + (open ? ' open' : '') },
            React.createElement('style', null, cardCss),
            React.createElement('button', {
              type: 'button',
              className: 'dsh-hud-card-header',
              'aria-expanded': open,
              'aria-controls': bodyId,
              onClick: function () { setOpen(!open) },
            },
              React.createElement('span', { className: 'dsh-hud-card-headText' },
                React.createElement('span', { className: 'dsh-hud-card-name' }, 'Game HUD'),
                React.createElement('span', { className: 'dsh-hud-card-description' }, '游戏化悬浮窗：余额血条、上下文蓝条、峰谷定价与低余额告警。'),
              ),
              dirty ? React.createElement('span', { className: 'dsh-hud-card-pending' }, '未保存') : null,
              React.createElement('span', { className: 'dsh-hud-card-chevron' + (open ? ' open' : '') }),
            ),
            open ? React.createElement('div', { className: 'dsh-hud-card-body', id: bodyId },
              !writable ? React.createElement('p', { className: 'dsh-hud-card-msg err' }, '本部署的设置为只读。') : null,
              React.createElement('div', { className: 'dsh-hud-card-row' },
                React.createElement('label', { className: 'dsh-hud-card-label', htmlFor: 'dsh-hud-settings-balance' }, '血条满格金额（¥）'),
                React.createElement('input', {
                  id: 'dsh-hud-settings-balance',
                  className: 'dsh-hud-card-input',
                  type: 'number',
                  min: '0.01',
                  step: '1',
                  value: draft,
              ...
              React.createElement('div', { className: 'dsh-hud-card-row' },
                React.createElement('label', { className: 'dsh-hud-card-label', htmlFor: 'dsh-hud-settings-threshold' }, '低余额红光告警'),
                React.createElement('div', {
                  className: 'dsh-hud-card-switch' + (alertOn ? ' on' : ''),
                  role: 'switch',
                  'aria-checked': alertOn,
                  onClick: function () { if (writable) setAlertOn(!alertOn) },
                }),
                React.createElement('label', { className: 'dsh-hud-card-label', htmlFor: 'dsh-hud-settings-threshold' }, '阈值 %'),
                React.createElement('input', {
                  id: 'dsh-hud-settings-threshold',
                  className: 'dsh-hud-card-input',
                  type: 'number',
                  min: '1',
                  max: '99',
                  ...
                  value: alertDraft,
                  disabled: !writable,
                  onChange: function (e) { setAlertDraft(e.target.value) },
                }),
              ),
              React.createElement('p', { className: 'dsh-hud-card-hint' }, '余额（血条）低于阈值时，悬浮窗四周闪烁红光提示；蓝条（上下文）不触发。'),
              savedMsg ? React.createElement('p', { className: 'dsh-hud-card-msg ok' }, savedMsg) : null,
              error ? React.createElement('p', { className: 'dsh-hud-card-msg err' }, error) : null,
              React.createElement('div', { className: 'dsh-hud-card-footer' },
                React.createElement('button', {
                  type: 'button',
                  className: 'dsh-hud-card-discard',
                  disabled: !dirty || saving,
                  onClick: onDiscard,
                }, '放弃修改'),
                React.createElement('button', {
                  type: 'button',
                  className: 'dsh-hud-card-save',
                  disabled: !dirty || saving,
                  onClick: onSave,
                }, saving ? '保存中…' : '保存'),
              ),
        ...
        slotsSvc.inject('settings.plugin.item', function () {
          return slotsSvc.register(
            { name: 'settings.plugin.item', key: 'game-hud', id: 'dsh-game-hud', order: 60 },
            function () {
              return React.createElement(HudSettingsCard)
            },
          )
        })
      }
    ...
    exports.apply = apply
    exports.inject = ['slots', 'sessions', 'timer', 'settingsScope']
