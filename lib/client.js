  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      function Hud(props) {
        ...
        const hpPct = balanceOk ? Math.max(0, Math.min(100, (balanceTotal / MAX_BALANCE) * 100)) : 0
          ...
          minimized ? null : React.createElement('div', { className: 'dsh-hud-body' },
            React.createElement('div', { className: 'dsh-hud-row' },
              React.createElement('div', { className: 'dsh-bar-label' },
                ...
                React.createElement('span', null, balanceOk ? ('¥' + balanceTotal.toFixed(2) + ' / ¥' + MAX_BALANCE + '.00') : '¥ --'),
              ),
