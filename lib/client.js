    ...
    function apply(ctx) {
      ...
      if (settingsScopeSvc !== undefined) {
        ...
        function HudSettingsCard() {
          ...
          const cardCss = '.dsh-hud-settings{display:flex;flex-direction:column;gap:10px;padding:4px 2px;font-family:inherit;font-size:12px;color:#d8e7f6}' +
            '.dsh-hud-settings .row{display:flex;align-items:center;gap:8px;flex-wrap:wrap}' +
            '.dsh-hud-settings label{white-space:nowrap;color:rgba(216,231,246,.75)}' +
            '.dsh-hud-settings input{width:90px;padding:4px 6px;border-radius:6px;border:1px solid rgba(90,220,255,.4);background:rgba(10,14,26,.9);color:#d8e7f6;font-family:inherit;font-size:12px}' +
            '.dsh-hud-settings button{padding:4px 14px;border-radius:6px;border:1px solid rgba(90,220,255,.45);background:rgba(0,180,255,.15);color:#d8e7f6;font-family:inherit;font-size:12px;cursor:pointer}' +
            '.dsh-hud-settings button:hover{background:rgba(0,180,255,.28)}' +
            '.dsh-hud-settings button:disabled{opacity:.6;cursor:wait}' +
            '.dsh-hud-settings .msg{font-size:11px}' +
            '.dsh-hud-settings .ok{color:#7be3a8}' +
            '.dsh-hud-settings .err{color:#ff7a8f}' +
            '.dsh-hud-settings .switch{position:relative;width:36px;height:20px;border-radius:10px;background:rgba(255,45,85,.25);border:1px solid rgba(255,45,85,.5);cursor:pointer;transition:background .2s}' +
            '.dsh-hud-settings .switch.on{background:rgba(255,45,85,.75)}' +
            '.dsh-hud-settings .switch::after{content:"";position:absolute;top:2px;left:2px;width:14px;height:14px;border-radius:50%;background:#fff;transition:left .2s}' +
            '.dsh-hud-settings .switch.on::after{left:18px}' +
            '.dsh-hud-settings .hint{font-size:11px;color:rgba(216,231,246,.55)}'

          return React.createElement('div', { className: 'dsh-hud-settings' },
            React.createElement('style', null, cardCss),
            React.createElement('div', { className: 'row' },
              React.createElement('label', null, '血条满格金额（¥）'),
            ...
            React.createElement('div', { className: 'row' },
              React.createElement('label', null, '低余额红光告警'),
              React.createElement('div', {
                className: 'switch' + (alertOn ? ' on' : ''),
                onClick: function () { setAlertOn(!alertOn) },
              }),
            ...
            React.createElement('div', { className: 'row' },
              React.createElement('button', { disabled: saving, onClick: onSave }, saving ? '保存中…' : '保存'),
            ),
            React.createElement('div', { className: 'hint' }, '余额（血条）低于阈值时，悬浮窗四周闪烁红光提示；蓝条（上下文）不触发。'),
            savedMsg ? React.createElement('div', { className: 'msg ok' }, savedMsg) : null,
            error ? React.createElement('div', { className: 'msg err' }, error) : null,
          )
        }
